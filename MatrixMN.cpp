...
void MatrixMN<T>::cout()
{
    for (int row = 0; row < num_rows_; row++)
    {
        for (int col = 0; col < num_cols_; col++)
        {
            std::cout << std::fixed << std::setw(12) << std::setprecision(8) << getValue(row, col) << " ";
        }
...
template<class T>
void MatrixMN<T>::delete_row(const int &row_index) {
    assert(row_index <= num_rows_ - 1);

    const int num_all = (num_rows_ - 1) * num_cols_;

    T* temp = new T[num_all];
...
template<class T>
void MatrixMN<T>::delete_col(const int &col_index) {
    assert(col_index <= num_cols_ - 1);

    T* new_values = new T[num_rows_ * (num_cols_ - 1)];
    ...
    for(int r=0;r<num_rows_;r++) {
        for(int c=0;c<num_cols_;c++) {
            ...
            new_values[index++] = values_[num_cols_ * r + c];
        }
    }
